...
//------------------------------------------
  ...
  for (i = 0; i < 0x40; i++) { //code modified 
  ...
  // Free the page allocated for the system stack
   pcb->npages=0;// unmapping the pages from the process
...
//------------------------------------------------------------
  ...
  for (i=0; i<0x40; i++)
  {
     pcb->pagetable[i] = 0x00;
  }

  // Code for the memory allocation for data and text segments 
 ...
 // user stack initization 
  newPage = MemoryAllocPage ();
  if (newPage == 0) {
    printf ("aFATAL: couldn't allocate memory - no free pages!\n");
    exitsim ();	// NEVER RETURNS!
  }
  pcb->pagetable[0x40 -1 ] = MemorySetupPte (newPage);

  // System Stack initilization
  newPage = MemoryAllocPage ();
  if (newPage == 0) {
...
//----------------------------------------------
  ...
  stackframe[PROCESS_STACK_PTSIZE] = 0x40;
...
void ProcessKill (PCB *pcb)
{
	// add your code below
	  ...
	  QueueRemove(&pcb->l);        // remove the process from the Queue
          ProcessFreeResources (pcb);  // Free the  pages when exits 
...
void PageFaultHandler()
{
  ...
  page_new = MemoryAllocPage();
  if ( page_new == 0){
  printf("Error, Page is not initialized properly\n");
  return  ProcessKill(currentPCB);
  }

  page_fault = faultaddress / MEMORY_PAGE_SIZE;
  currentPCB->pagetable[page_fault]= MemorySetupPte(page_new);
  currentPCB->npages = currentPCB->npages +1;


 printf("Process id %lu,\tpage fault address: %p\tPhysical page number allocated: %d\n",findpid(currentPCB),faultaddress,page_fault);
